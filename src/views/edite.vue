				...
				<el-main :id="mainId" :style="{ overflow: designData.isAuto ? 'hidden' : 'auto' }">
					...
					<div class="wrap" @dragover="allowDrop" @drop="drop" :style="[setWrap_style]"
						...
						<div v-for="(item, index) in list" :key="index" class="cptDiv" @mousedown="selectItem(index)"
							...
							<div class="component_main" :style="{
                  border: currentIndex === index ? '1px solid #B6BFCE' : '',
								...
								<el-icon @click.stop="edite(index)" v-show="currentIndex === index" style="
                    position: absolute;
                    color: #fff;
                    top: 0px;
                    right: 0px;
				...
				<el-form ref="formRef" label-width="100px" label-position="left">
					...
					<el-form-item label="背景图片">
						<div class="uploadItem" v-if="designData.bgImg" @click="upBgImg">
							...
							<el-image style="width: 100%; height: 100%" :src="designData.bgImg" fit="cover" />
						</div>
			...
			setWrap_style() {
				...
				let result = {
					...
					background: designData.bgColor ?
						designData.bgColor : "url(" + defaultBg + ") repeat",
