	...
	<div>
		<el-form label-width="80px" size="mini">
			<el-form-item label="切换触发">
				<el-select v-model="data.trigger" placeholder="请选择触发切换方式">
					<el-option label="hover" value="hover" />
					<el-option label="click" value="click" />
				</el-select>
			</el-form-item>
			<el-form-item label="图片填充">
				<el-select v-model="data.fit" placeholder="请选择图片填充方式">
			...
			<el-form-item label="轮播图片">
				<el-upload :action="fileUrl+'/file/upload?dir=imgPool'" :headers="uploadHeaders"
					:before-upload="beforeBgImgUpload" :show-file-list="false" :on-error="handleError"
					list-type="picture" :on-success="handleAvatarSuccess">
					<el-button size="small" type="primary">点击上传</el-button>
				</el-upload>
			</el-form-item>
			<el-row :gutter="4">
				<el-col :span="12" v-for="(item,index) in data.imgUrls" :key="index" class="imgBlock">
	...
	export default {
		...
		methods: {
			...
			handleAvatarSuccess(res) {
				...
				this.data.imgUrls.push(this.fileUrl + '/file/img/' + res.data);
			},
			handleError(err, file) {
				this.$message.error("图片上传失败，使用本地路径")
				this.data.imgUrls.push(file.url);
			}
		}
	}
</script>
